...
def total_test():
    ...
    ## veryfying with Hard test set
    # Enroll each FP in FP dataset/Real
    enrolling_folder_path = "DB1_B_Converted copy/" # "FP dataset/Real/"
    # Veryfy each FP in FP dataset/Test/Hard 
    verifying_folder_path = "DB1_B_Converted copy/" # "FP dataset/Test/Hard/"
    
    logging_file_name = "DB1_B" # "Hard result"
    ...
    for enrolling_file in os.listdir(enrolling_folder_path):
        ...
        # enroll FP
        good_fp = False
        print(str(hard_count) + "Enroll : " + enrolling_file)
        good_fp = capture_new_fp_xyt(enrolling_folder_path + enrolling_file, "0")

        if not good_fp:
            with open("Unenrolment", 'a') as file:
                    file.write('{}\n'.format(enrolling_file))
            print(str(hard_count) + " Enroll : " + enrolling_file + " Failed")
            hard_count += 1
            continue
        ...
        gac = 0
        tot = 0
        print("Start verifying for ", enrolling_file)
        for verifying_file in os.listdir(verifying_folder_path):
            # verifying_file_name = verifying_file.split("finger_")[0] + "finger"
            verifying_file_name = verifying_file.split(".")[0]
            print(verifying_file)
            if(enrolling_file_name.split("_")[0]==verifying_file_name.split("_")[0]):
                count = count + 1
                tot = tot + 1
